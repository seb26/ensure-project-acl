...
class Ace:
    def __init__(
        self, principal_type, name, access, rights, apply_to, level=0, index=None
    ):
...
class Acl:
    def __init__(self, path):
        ...
        self.entries = []
        self.loaded = False
        self.ace_pattern = re.compile(
            r"^\s*\[(\d+)\]\s+(user|group|owner|everyone|authenticated_user|system):(.*?):(allow|deny):([rwxpdDaARWcCo-]+):([fdin-]+)\s+\(level:(\d+)\)"
        )
        if not self.load():
    ...
    def _synoacltool(self, args, check=True):
        ...
        try:
            ...
            logger.debug(
                f"synoacltool returned: code={result.returncode}, stdout={result.stdout.strip()!r}, stderr={result.stderr.strip()!r}"
            )
    ...
    def load(self):
        ...
        if result.returncode != 0:
            ...
            if result.returncode == 255 and ("Linux mode" in stdout or stdout == ""):
                logger.log(
                    NOTICE,
                    f"folder {self.path} is in Linux mode - initializing ACL with inheritance",
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"captured POSIX mode {oct(original_posix_mode)} for {self.path}"
                    )
                except OSError as e:
                ...
                if original_posix_mode is not None:
                    try:
                        ...
                        logger.debug(
                            f"restored POSIX mode {oct(original_posix_mode)} for {self.path}"
                        )
                    except OSError as e:
                        logger.warning(
                            f"failed to restore POSIX mode for {self.path}: {e}"
                        )
                ...
                if not reload_result or reload_result.returncode != 0:
                    logger.error(
                        f"failed to reload ACL after enforce-inherit for {self.path}"
                    )
                    self.loaded = False
                    return False
                ...
                for line in reload_result.stdout.splitlines():
                    ...
                    if match:
                        idx, p_type, name, access, perms, inherit, level = (
                            match.groups()
                        )
                        self.entries.append(
                            Ace(p_type, name, access, perms, inherit, level, idx)
                        )
                logger.log(
                    NOTICE,
                    f"initialized ACL for {self.path} with {len(self.entries)} inherited entries",
                )
                self.loaded = True
                return True
            else:
        ...
        for line in result.stdout.splitlines():
            ...
            if match:
                ...
                stripped = line.strip()
                if (
                    stripped.startswith("ACL version:")
                    or stripped.startswith("Archive:")
                    or stripped.startswith("Owner:")
                    or stripped == "---------------------"
                ):
                    continue
    ...
    def sync_ace(self, target_ace):
        ...
        all_explicit = [
            ...
            if e.name == target_ace.name
            and e.principal_type == target_ace.principal_type
            and e.level == 0
        ]
        if not all_explicit:
        ...
        if len(all_explicit) > 1:
            ...
            extras_sorted = sorted(
                all_explicit[1:], key=lambda e: int(e.index), reverse=True
            )
            ...
            if not self.load():
                raise RuntimeError(
                    f"failed to reload ACL after deletion for {self.path}"
                )
            all_explicit = [
                e
                for e in self.entries
            ...
            if not all_explicit:
                logger.warning(
                    f"ACE disappeared after cleanup, re-adding for {self.path}"
                )
                self._add_ace(target_ace)
                return True
            ...
            if (
                primary.perms != target_ace.perms
                or primary.inherit != target_ace.inherit
                or primary.access != target_ace.access
            ):
        ...
        if (
            ...
            return True
        logger.debug(
            f"no change needed: {target_ace.principal_type}:{target_ace.name} on {self.path}"
        )
        return False
    ...
    def _add_ace(self, target_ace) -> None:
        ...
        logger.log(
            NOTICE,
            f"added {target_ace.principal_type}:{target_ace.name} to {self.path}",
        )
        # Update local state so subsequent sync_ace calls see this ACE
        added_ace = Ace(
            target_ace.principal_type,
            target_ace.name,
            target_ace.access,
            target_ace.perms,
            target_ace.inherit,
    ...
    def _replace_ace(self, index: str, target_ace) -> None:
        ...
        result = self._synoacltool(
            ["-replace", self.path, str(index), target_ace.to_syno_str()]
        )
        if result is None:
            raise RuntimeError(
                f"failed to replace ACE at index {index} for {self.path}"
            )
        logger.log(
            NOTICE,
            f"updated {target_ace.principal_type}:{target_ace.name} on {self.path}",
        )
